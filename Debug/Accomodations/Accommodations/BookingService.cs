...
public class BookingService : IBookingService
{
    ...
    {
        //»зменил условие с "меньше" на "меньше или равно" и добавил в вывод, что даты не должны совпадать 
        if ( endDate <= startDate )
        {
            throw new ArgumentException( "End date cannot be earlier than start date or equals" );
        }
        else if ( startDate <= DateTime.Now ) // ƒобавил проверку на начало бронировани€
        {
    ...
    public IEnumerable<Booking> SearchBookings( DateTime startDate, DateTime endDate, string categoryName )
    {
        ...
        // —делал включительно до даты выезда
    ...
    public decimal CalculateCancellationPenaltyAmount( Booking booking )
    {
        ...
        // ѕомен€л местами даты, чтобы не было отрицательного числа
        int daysBeforeArrival = ( booking.StartDate - DateTime.Now ).Days;

        // ≈сли забронировали на следующий день, то вернЄм фиксированную сумму без вычислений
        if ( daysBeforeArrival == 0 )
        {
            return 5000.0m;
        }

        // »зменил вывод числа только до сотых
        return Math.Round( ( 5000.0m / daysBeforeArrival ), 2 );
    }
    ...
    private static decimal CalculateBookingCost( decimal baseRate, int days, int userId, decimal currencyRate )
    {
        ...
        // —делал умножение на курс уже на финальную цену
